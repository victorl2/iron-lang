...
| 14 | src/comptime/comptime.c:410-412 | Integer Safety | DONE — Phase 67-03 | commit `1220839` (feat(67-03): overflow-safe comptime arithmetic + INT64_MIN negation guards (ranks 14+15)); IRON_CVAL_INT binop switch now uses `__builtin_add_overflow` / `__builtin_sub_overflow` / `__builtin_mul_overflow` for `+`, `-`, `*` and routes overflow through `emit_error(ctx, IRON_ERR_COMPTIME_ERROR, ..., "comptime: integer overflow in {addition,subtraction,multiplication}")`; SLASH and PERCENT cases gained explicit `INT64_MIN / -1` guards for the C11 6.5.5p6 UB. grep `__builtin_add_overflow` comptime.c → 1 call-site hit, grep `INT64_MIN` → 3 guard sites (div/mod/negation); integration fixtures `int_comptime_arith_overflow.iron` + `int_comptime_neg_min.iron` exercise the non-overflow path on every build | — |
| 15 | src/comptime/comptime.c:493 | Integer Safety | DONE — Phase 67-03 | Same commit `1220839`; IRON_TOK_MINUS unary branch in `iron_comptime_eval_expr` now explicitly rejects `operand->as_int == INT64_MIN` before negating, routing through `emit_error(ctx, IRON_ERR_COMPTIME_ERROR, ..., "comptime: integer overflow in negation (INT64_MIN)")`; fixture `int_comptime_neg_min.iron` exercises near-min negation paths that MUST stay in range | — |
...
| 19 | src/parser/parser.c:2585 | Integer Safety | DONE — Phase 67-03 | commit `1372702` (feat(67-03): atoi -> strtol for enum variant values + fixture (rank 19)); enum-variant explicit-value path in `iron_parse_decl` now calls `strtol(num->value, &end, 10)` with `errno == ERANGE` + `INT_MIN <= parsed <= INT_MAX` bounds + endptr-consumed check; out-of-range values route through `iron_emit_diag(p, IRON_ERR_UNEXPECTED_TOKEN, ..., "enum variant value '%s' out of range (expected int32 in [INT_MIN, INT_MAX])")` with a defensive fallback of `v->explicit_value = 0`. grep `atoi(num->value)` parser.c → 0 hits; grep `strtol` → 1 hit; grep `ERANGE` → 1 hit. Fixture `int_enum_value_overflow.iron` exercises an enum variant at exactly INT_MAX = 2147483647 | — |
...
**Summary:** 20 DONE (ranks 1-20, all top-20 rows closed), 0 OPEN. Phase 67-03 closed ranks 14, 15, 19 (integer-safety tail). All subsequent Phase 67 plans (67-04..67-08) address non-top-20 M-severity rows and REG-02 canaries.
...
| 14 | src/comptime/comptime.c:410-412 | 67-03 | Integer-overflow arithmetic — wrap in `__builtin_add/sub/mul_overflow` with error emission | DONE — commit `1220839` |
| 15 | src/comptime/comptime.c:493 | 67-03 | INT64_MIN negation UB — explicit check before negation | DONE — commit `1220839` |
| 19 | src/parser/parser.c:2585 | 67-03 | `atoi` → `strtol` with ERANGE + INT_MIN/INT_MAX bounds for enum variant values | DONE — commit `1372702` |
