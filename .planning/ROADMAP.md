...
- v0.2.0-alpha Networking Standard Library - Phases 59-64 (paused)
- v0.1.4-alpha Compiler Correctness & Maintenance - Phases 65-70 (active)
...
### v0.2.0-alpha Networking Standard Library

- [x] **Phase 59: Net Foundation + Windows CI + URL** - TCP/UDP/IP/DNS primitives, cross-platform timeouts via non-blocking I/O + poll/WSAPoll, Windows CI runner, URL parse/build/resolve in pure Iron (completed 2026-04-11)
- [ ] **Phase 60: [paused] TLS Integration** - mbedTLS 3.6.6 LTS vendored, required hostname verification, Mozilla CA bundle, TLS 1.2+1.3 only
- [ ] **Phase 61: [paused] JSON** - cJSON vendored, Value sum type, parse/stringify/decode<T>/encode<T>, depth/size DoS limits, JSON Schema 2020-12 subset validator
- [ ] **Phase 62: [paused] HTTP/1.1 Parser + Client** - Shared parser, client surface (get/post/put/delete/request), HTTPS via TLS, redirect chasing with cross-host auth stripping, CL/TE smuggling rejection
- [ ] **Phase 63: [paused] HTTP Server** - Return-value handlers, Go 1.22-style routing, HTTPS via listen_tls, Slowloris defense (header/body timeouts + size caps), smuggling rejection, spawn-per-connection
- [ ] **Phase 64: [paused] WebSocket Client** - ws:// and wss:// connect, text/binary frames, fragmented messages, ping/pong auto-response, RFC 6455 close handshake, max frame size cap
...
- [ ] **Phase 70: Version Reconciliation** - Canonical version scheme documented, planning docs synced, CI release-tag validation
...
**Execution Order:**
Phases execute in numeric order: 59 -> 65 -> 66 -> 67 -> 68 -> 69 -> 70 (60-64 paused)
...
| 59. Net Foundation + Windows CI + URL | 9/9 | Complete    | 2026-04-11 | - |
| 60. [paused] TLS Integration | v0.2.0-alpha | 0/0 | Paused | - |
| 61. [paused] JSON | v0.2.0-alpha | 0/0 | Paused | - |
...
| 66. Structural Protections + Linux Release CI | 2/5 | In Progress|  | - |
...
### v0.2.0-alpha Networking Standard Library Phase Details

### Phase 59: Net Foundation + Windows CI + URL
**Goal**: Iron programs can open TCP and UDP sockets, resolve hostnames, parse and build URLs, and run correctly on macOS, Linux, and Windows with real per-call timeouts
**Depends on**: Phase 58
**Requirements**: INFRA-04, INFRA-05, INFRA-06, INFRA-07, INFRA-08, INFRA-09, INFRA-10, NET-01, NET-02, NET-03, NET-04, NET-05, NET-06, NET-07, NET-08, NET-09, NET-10, NET-11, NET-12, NET-13, URL-01, URL-02, URL-03, URL-04, URL-05, URL-06, URL-07
**Success Criteria** (what must be TRUE):
  1. A user can write `val sock, err = net.tcp_dial("example.com", 80, timeout: 5000)` in Iron, build with `iron build`, run the resulting binary on macOS, Linux, **and** Windows, and observe a connected socket or a typed timeout error; the same program using `net.tcp_listen` + `accept` serves a client from a second process on the same machine
  2. A user can write `val addrs, err = net.lookup_host("example.com", timeout: 2000)` and receive one or more typed `Address` values (IPv4 or IPv6); a deliberately unreachable resolver produces a typed timeout error within the requested deadline, not a hang
  3. A user can parse an IPv6 bracketed URL like `https://[2001:db8::1]:8443/path?q=1#frag` via `url.parse(str)`, access every component (scheme, host, port, path, query, fragment) as typed fields, build a new URL with `Url.builder()`, and resolve a relative URL against a base with `url.resolve(base, rel)` -- all via `import url` in pure Iron source
  4. A user's Iron program that writes to a closed TCP socket returns a typed `Iron_Error` instead of being killed by SIGPIPE, and a dual-stack listener accepts both IPv4 and IPv6 clients on the same port without extra configuration
  5. The Windows GitHub Actions CI runner builds the Iron toolchain, runs every networking integration test, and reports green on every commit to `main` -- cross-platform networking regressions cannot land undetected
**Plans**: 9/9 plans complete (the monolithic 59-01 was split into 59-01a/b/c/d during execution to isolate the foundation, pool, runtime hooks, and tuple-return codegen tasks)

Plans:
- [x] 59-01a-PLAN.md — iron_errors.h (net+url codes) + Iron_Deadline helper (completed 2026-04-10)
- [x] 59-01b-PLAN.md — Iron_Pool elastic extension + Iron_io_pool + Iron_PoolWait abandoned-flag API (completed 2026-04-10)
- [x] 59-01c-PLAN.md — iron_runtime_init WSA hooks + SIGPIPE ignored + String primitives (rindex_of / byte_at / from_byte) + Duration + dual build-path wiring (completed 2026-04-10)
- [x] 59-01d-PLAN.md — Tuple-return codegen (2-element tuples through hir_to_lir + emit_c.c) (completed 2026-04-10)
- [x] 59-02-PLAN.md — TCP surface: iron_net.{h,c} dial/listen/accept/read/write/close, non-blocking + poll/WSAPoll wrapper, errno+WSA translation tables, IPV6_V6ONLY=0 dual-stack, CMake + src/cli/build.c wiring for ws2_32/iphlpapi (completed 2026-04-10)
- [x] 59-03-PLAN.md — UDP + IP address types: Net.udp_bind/sendto/recvfrom, IPv4Addr/IPv6Addr parse+format via inet_pton/inet_ntop, enum Address ADT in net.iron, zone identifier handling (completed 2026-04-10)
- [x] 59-04-PLAN.md — DNS: Net.lookup_host on Iron_io_pool via getaddrinfo, abandoned-flag stuck-worker pattern, condvar-deadline wait, localhost + synthetic slow-worker tests (completed 2026-04-11)
- [x] 59-05-PLAN.md — Pure-Iron URL module: stdlib/url.iron with parse/build/resolve/percent-encode/decode/default_port, IPv6 bracketed hosts, RFC 3986 §5.4 reference resolution, import url wiring in build.c + check.c (completed 2026-04-10)
- [x] 59-06-PLAN.md — Windows CI enablement: ci.yml windows-latest matrix + remove continue-on-error, cross-platform Unity battery (test_stdlib_net_cross_platform), dual-stack + full-roundtrip Iron integration tests (completed 2026-04-11)

### Phase 60: [paused] TLS Integration
**Goal**: Iron programs can make TLS connections to servers with full chain and hostname verification by default, using a self-contained vendored mbedTLS and CA bundle that ships with the Iron distribution
**Depends on**: Phase 59
**Requirements**: INFRA-01, INFRA-03, TLS-01, TLS-02, TLS-03, TLS-04, TLS-05, TLS-06, TLS-07
**Success Criteria** (what must be TRUE):
  1. An Iron program can wrap a TCP socket in a TLS client stream via an internal `iron_tls` runtime API that **requires** the hostname as a non-nullable parameter; connecting to `https://wrong.host.badssl.com` produces a typed hostname-verification error, and connecting to `https://expired.badssl.com` produces a typed expired-certificate error
  2. Chain verification against the vendored Mozilla NSS CA bundle succeeds for mainstream public hosts (`www.google.com`, `api.github.com`, `cloudflare.com`) on macOS, Linux, and Windows with no system TLS dependency -- the CA bundle ships at `certs/ca-bundle.crt` and is located via `IRON_CERTS_DIR` compile-time discovery across install prefix, dev tree, and user override
  3. Attempting to negotiate SSLv3, TLS 1.0, or TLS 1.1 fails because those versions are disabled at compile time; only TLS 1.2 and TLS 1.3 handshakes succeed
  4. A TLS server context loads a PEM certificate + key pair from disk and accepts a TLS 1.3 client connection from a second Iron process; the mbedTLS entropy source is initialized exactly once per process from `Iron_runtime_init`, not per-connection
  5. No raw mbedTLS symbol or header is reachable from Iron user code -- the only TLS API is `iron_tls.{h,c}` internal to the runtime, and a grep-test in CI fails the build if a public Iron stdlib module imports mbedtls directly
**Plans**: TBD

### Phase 61: [paused] JSON
**Goal**: Iron programs can parse, stringify, decode into typed objects, encode from typed objects, and validate JSON against a schema, with DoS-safe defaults and typed errors that point to the offending path
**Depends on**: Phase 60
**Requirements**: INFRA-02, JSON-01, JSON-02, JSON-03, JSON-04, JSON-05, JSON-06, JSON-07, JSON-08, JSON-09, JSON-10, JSON-11, JSON-12, JSON-13, JSON-14
**Success Criteria** (what must be TRUE):
  1. A user can write `val value, err = json.parse(str)` and receive a typed `json.Value` sum type (Null, Bool, Number, String, Array, Object) with typed accessors (`get`, `index`, `as_string`, `as_int`, `as_float`, `as_bool`); `json.stringify(value)` round-trips the result without silent integer-to-double coercion for int64 values
  2. A user can write `val user, err = json.decode<User>(str)` where `User` is an Iron object type and receive a fully typed instance **or** a typed error carrying the JSON path of the mismatch (e.g., `expected int at $.user.age`); `json.encode<User>(value)` produces a string that round-trips through `decode<User>`
  3. Parsing a document with duplicate keys in an object produces a typed error by default (security-positive); parsing a 17 MB document or one with 129 levels of nesting is rejected with a typed DoS-prevention error before `cJSON_Parse` is called; a libFuzzer run against the parser in CI completes without segfault, heap overflow, or uncaught abort
  4. A user can define a `json.Schema` in Iron source (types, required, properties, items, minimum/maximum, pattern, enum -- JSON Schema 2020-12 subset), call `json.validate(value, schema)`, and receive a list of typed errors where each entry carries a JSON path and a human-readable message for every failure
  5. The `iron_stdlib` target links against cJSON 1.7.19 vendored at `src/vendor/cjson/`, with the wrapper logic in `iron_json.{h,c}` and the pure-Iron surface in `stdlib/json.iron`; user code reaches everything via `import json` with no C-level exposure
**Plans**: TBD

### Phase 62: [paused] HTTP/1.1 Parser + Client
**Goal**: Iron programs can issue HTTP/1.1 requests (GET/POST/PUT/DELETE/custom) against plaintext and HTTPS servers with typed responses, redirect following, per-call timeouts, and defenses against request-smuggling and cross-host credential leakage
**Depends on**: Phase 61
**Requirements**: HCLI-01, HCLI-02, HCLI-03, HCLI-04, HCLI-05, HCLI-06, HCLI-07, HCLI-08, HCLI-09, HCLI-10, HCLI-11, HCLI-12
**Success Criteria** (what must be TRUE):
  1. A user can write `val resp, err = http.get("https://api.github.com/repos/iron-lang/iron", timeout: 5000)`, receive a typed response with status, headers (case-insensitive lookup), and body, and call `resp.json()` to parse the body via the JSON module -- all in a single Iron program with `import http`
  2. `http.post`, `http.put`, `http.delete`, and the general `http.request(method:, url:, headers:, body:, timeout:)` all work against both `http://` and `https://` URLs transparently; a per-call `timeout:` bounds the total time across connect, header read, and body read via monotonic deadlines, not per-syscall `SO_RCVTIMEO`
  3. Following a redirect from `host-a.com` to `host-b.com` strips `Authorization`, `Cookie`, and `Proxy-Authorization` headers; following a redirect on the same host preserves them; the default redirect cap is 10 and is configurable; an integration test with two local servers proves both behaviors
  4. A response with **both** `Content-Length` and `Transfer-Encoding` headers is rejected with a typed `Iron_Error` -- the client does not pick one and proceed (request-smuggling defense); a response with bare CR/LF, non-`chunked` Transfer-Encoding, or non-digit Content-Length is also rejected
  5. A user can pass `no_verify: true` as a dev escape hatch against a self-signed certificate and observe the request succeed; the default (no flag) fails against the same self-signed cert with a typed TLS verification error
**Plans**: TBD

### Phase 63: [paused] HTTP Server
**Goal**: Iron programs can serve HTTP/1.1 and HTTPS traffic with Go 1.22-style routing, return-value handlers, middleware composition, and built-in defenses against Slowloris, request smuggling, and unbounded headers/bodies
**Depends on**: Phase 62
**Requirements**: HSRV-01, HSRV-02, HSRV-03, HSRV-04, HSRV-05, HSRV-06, HSRV-07, HSRV-08, HSRV-09, HSRV-10, HSRV-11, HSRV-12, HSRV-13, HSRV-14, HSRV-15, HSRV-16
**Success Criteria** (what must be TRUE):
  1. A user can construct a server with `http.Server.new()`, register routes via `server.route("GET /users/{id}", handler)` using Go 1.22-style patterns (`{id}`, `{path...}`, `{$}`), call `server.listen("0.0.0.0", 8080)` or `server.listen_tls(...)`, and see an external HTTP/HTTPS client (curl or another Iron process) receive the response from a handler defined as `fn(Request) -> Response` -- no `ResponseWriter`, no "wrote headers already" bug class
  2. A handler can read path params via `request.path_param("id")`, query params via `request.query_param("q")`, and body as bytes, string, or parsed JSON via `request.body_json()`; responses are built via `Response.new(status).header(...).body(...)` or `Response.new(status).json(value)`, and `Content-Length` is auto-calculated when body size is known
  3. A client holding a TCP connection open without sending request headers is disconnected within 10 seconds (default `header_read_timeout`); a request with more than 64 KB of headers is rejected (default `max_header_bytes`); a request with a slow body stream is disconnected after 30 seconds (default `body_read_timeout`) or when more than 8 MB of body bytes arrive (default `max_body_bytes`) -- Slowloris integration test passes
  4. A request with both `Content-Length` and `Transfer-Encoding` headers is rejected with 400, never parsed via either; the server handles concurrent connections by spawning an Iron task per connection via the existing `spawn` runtime, and a handler that panics or returns an error converts into a 500 response without taking down the server
  5. Unmatched routes produce an automatic 404; method mismatches produce an automatic 405; a middleware chain built via `fn(next: Handler) -> Handler` composition applies in the declared order across all routes
**Plans**: TBD

### Phase 64: [paused] WebSocket Client
**Goal**: Iron programs can connect to `ws://` and `wss://` endpoints, exchange text and binary messages, handle fragmentation and ping/pong, and perform the RFC 6455 close handshake with per-call timeouts
**Depends on**: Phase 63
**Requirements**: WS-01, WS-02, WS-03, WS-04, WS-05, WS-06, WS-07, WS-08, WS-09, WS-10
**Success Criteria** (what must be TRUE):
  1. A user can write `val conn, err = ws.connect("wss://echo.websocket.events", timeout: 5000)` against a public `wss://` server, send a text frame via `conn.send_text("hello")`, and receive an echoed text frame via `conn.receive(timeout: 1000)` returning a typed `Frame::Text(msg)` -- the same program works against a `ws://` URL against a local plaintext server
  2. The handshake sends an RFC 6455 compliant `Sec-WebSocket-Key` and validates the server's `Sec-WebSocket-Accept`; outbound frames are masked per RFC 6455; a deliberately invalid `Sec-WebSocket-Accept` response causes `ws.connect` to fail with a typed error
  3. A text message split across multiple frames (fragmented) is reassembled into a single `Frame::Text` delivered to the user; an inbound text frame containing invalid UTF-8 is rejected per RFC 6455; an inbound frame larger than the 16 MB default max frame size is rejected with a typed error
  4. An inbound ping frame is answered automatically with a pong (keep-alive) without the user calling any API; `ws.close()` performs the RFC 6455 close handshake bounded by a timeout and returns cleanly even if the peer never sends a matching close
  5. A user's Iron program using `import ws` against a `wss://` URL works on macOS, Linux, and Windows transparently via the TLS module from Phase 60 -- no separate configuration or code path
**Plans**: TBD

### v0.1.4-alpha Compiler Correctness & Maintenance Phase Details

### Phase 65: Correctness Audit
**Goal**: The compiler, runtime, and stdlib C code have been systematically audited across 7 correctness dimensions, producing a ranked issue list that drives all subsequent phases
**Depends on**: Phase 59 (last shipped phase)
**Requirements**: AUDIT-01, AUDIT-02, AUDIT-03, AUDIT-04, AUDIT-05, AUDIT-06, AUDIT-07, AUDIT-08, AUDIT-09
**Success Criteria** (what must be TRUE):
  1. `.planning/research/CORRECTNESS-AUDIT.md` exists with one table per audit dimension (blind casts, enum exhaustiveness, null safety, arena lifetimes, integer safety, syscall error handling, cross-platform correctness) -- each row contains file:line, severity, description, suggested fix, and target regression fixture name
  2. Every compiler pass (parser through emit_c sub-modules) and every runtime/stdlib C file has been read for blind casts, and each unguarded `(Iron_XXX *)node` cast is documented with file:line and severity
  3. Every `switch` over an `Iron_*Kind` / `IronHIR_*Kind` / `IronLIR_OpKind` enum has been checked for exhaustiveness, and silent `default:` fall-throughs are documented with the specific enum values they miss
  4. The top-20 high-severity issues are flagged as "must-fix in this milestone" with clear enough descriptions that Phase 67 can resolve them without re-reading the source
  5. `.planning/research/CROSS-PLATFORM-DEBT.md` exists documenting all code that assumes 64-bit pointers, little-endian byte order, POSIX-only headers, GCC-only attributes, or specific struct padding rules
**Plans**: 6 plans
Plans:
- [ ] 65-01-PLAN.md -- Audit src/parser/ + src/lexer/ (all 8 dimensions)
- [ ] 65-02-PLAN.md -- Audit src/analyzer/ (all 8 dimensions)
- [ ] 65-03-PLAN.md -- Audit src/hir/ + src/comptime/ (all 8 dimensions)
- [ ] 65-04-PLAN.md -- Audit src/lir/ (all 8 dimensions)
- [ ] 65-05-PLAN.md -- Audit src/runtime/ + src/stdlib/ + src/cli/ + src/diagnostics/ + src/pkg/ + src/util/ (all 8 dimensions)
- [ ] 65-06-PLAN.md -- Consolidation: merge findings, rank severity, top-20 must-fix, write final documents

### Phase 66: Structural Protections + Linux Release CI
**Goal**: The blind-cast bug class is structurally prevented from re-landing via compile-time assertions, switch-enum enforcement, and runtime kind-checks; Linux Release CI catches cross-platform crashes immediately
**Depends on**: Phase 65
**Requirements**: PROT-01, PROT-02, PROT-03, PROT-04, REG-01, REG-04
**Success Criteria** (what must be TRUE):
  1. `_Static_assert` in `src/parser/ast.h` enforces the `{Iron_Span span; Iron_NodeKind kind; struct Iron_Type *resolved_type;}` prefix on every expression AST type -- adding a new expression node without the correct prefix fails the build at compile time
  2. Building `iron_compiler`, `iron_runtime`, `iron_stdlib`, and `ironc` targets with CMake succeeds with `-Werror=switch-enum` enabled -- every switch over an Iron enum either handles every case explicitly or has a commented `default:` explaining the opt-out
...
**Depends on**: Phase 66 (structural protections landed first so fixes use the new idioms)
**Requirements**: FIX-01, FIX-02, FIX-03, FIX-04, REG-02
**Success Criteria** (what must be TRUE):
  1. All 20 high-severity issues from `CORRECTNESS-AUDIT.md` are resolved -- each fix lands with a regression fixture (integration test, unit test, or C-level assertion) that would have failed before the fix and passes after
  2. Every unchecked `malloc`/`calloc`/`realloc`/`strdup` call that could crash on failure is either guarded with an error path or annotated `/* SAFETY: cannot fail because ... */` with the invariant documented at the call site
...
**Depends on**: Phase 65 (needs audit to identify parser/typecheck/hir hotspots; no structural dependency on 66/67)
**Requirements**: FUZZ-01, FUZZ-02, FUZZ-03, FUZZ-04, FUZZ-05, FUZZ-06
**Success Criteria** (what must be TRUE):
  1. `tests/fuzz/fuzz_parser.c` drives `iron_parse()` on random byte inputs under libFuzzer, crashes or hangs (>10s) are caught, and minimized crashes land in `tests/fuzz/corpus/parser/`
  2. `tests/fuzz/fuzz_typecheck.c` and `tests/fuzz/fuzz_hir_to_lir.c` exercise their respective pipeline stages with randomly-generated inputs seeded from the existing `tests/integration/*.iron` fixtures
  3. `.github/workflows/fuzz.yml` runs each libFuzzer target nightly for 10 minutes on `main`, uploads crashes as artifacts, and opens a GitHub issue tagged `fuzz-crash` when a regression is found
  4. `scripts/fuzz_crash_to_fixture.sh` auto-generates a minimal integration fixture from any fuzzer-found crash, so every crash accumulates as a permanent regression test
**Plans**: TBD

### Phase 69: Coverage Tooling + Benchmark Calibration
**Goal**: The team can measure per-file line and branch coverage, CI reports coverage deltas on PRs, files below 50% get targeted tests, and cross-platform benchmark thresholds are calibrated and documented
**Depends on**: Phase 65 (audit identifies low-coverage files; no structural dependency on 66-68)
**Requirements**: COV-01, COV-02, COV-03, COV-04, COV-05, REG-03
**Success Criteria** (what must be TRUE):
  1. `cmake -DIRON_ENABLE_COVERAGE=ON` adds coverage instrumentation to all Iron targets (clang-only); `scripts/coverage.sh` runs the test suite and produces both `build/coverage/index.html` and `build/coverage/summary.json`
  2. `.github/workflows/coverage.yml` runs on push to `main`, uploads the HTML report, and comments the per-file coverage delta on the landing PR
  3. `.planning/research/COVERAGE-BASELINE.md` lists per-file line and branch coverage for all files in `src/parser/`, `src/analyzer/`, `src/hir/`, `src/lir/`, `src/runtime/`, `src/stdlib/`, with files below 50% flagged
  4. Every file flagged as below 50% receives at least one targeted test raising it above 50%
  5. `docs/benchmark-calibration.md` documents the macOS-arm64-vs-Linux-x86_64 variance observed during Phase 59; `scripts/bench_audit.sh --platform linux-x86_64` re-audits thresholds from observed data
**Plans**: TBD

### Phase 70: Version Reconciliation
**Goal**: The internal planning-doc version scheme and the public GitHub release version scheme are reconciled, documented, and enforced by CI
**Depends on**: Nothing (housekeeping, can run any time after Phase 65)
**Requirements**: VER-01, VER-02, VER-03
**Success Criteria** (what must be TRUE):
  1. `docs/versioning.md` records the canonical scheme decision, the rationale, how the drift happened, and rules for future releases
  2. `.planning/PROJECT.md`, `.planning/ROADMAP.md`, `.planning/REQUIREMENTS.md`, and every phase folder name in `.planning/phases/` all use the canonical scheme
  3. The CI release-tag flow (`.github/workflows/changelog-on-release.yml` + `CMakeLists.txt` `IRON_VERSION_FULL` + `scripts/update_changelog_from_release.py`) rejects a release tag that diverges from the canonical scheme; a failure test case is documented
**Plans**: TBD
