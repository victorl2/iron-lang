...
- [x] **WEB-RUNTIME-04**: Analyzer emits an error if `await` is reachable from `Iron_main()` when `--target=web`
- [x] **WEB-RUNTIME-05**: New file `src/stdlib/iron_time_web.c` provides `Iron_time_now()`, `Iron_time_now_ms()`, `Iron_time_now_ns()` via `emscripten_get_now()`
- [x] **WEB-RUNTIME-06**: Web build links `iron_time_web.c` instead of `iron_time.c` (no touches to `iron_time.c`)
...
- [x] **WEB-EMIT-01**: New LIR pass `src/lir/web_main_loop_split.c` detects the canonical `while (!WindowShouldClose()) { body }` pattern in any function containing `InitWindow`
...
- [x] **WEB-BUILD-02**: `find_emcc()` probes PATH for `emcc`, `emcc.bat`, `emcc.cmd` (Windows compatibility)
...
- [x] **WEB-BUILD-05**: Raylib is built from the amalgamation driver `src/vendor/raylib/raylib.c` with `-DPLATFORM_WEB -DGRAPHICS_API_OPENGL_ES2` (NOT via raylib's Makefile)
...
- [x] **WEB-SHELL-06**: Custom `[web].shell` templates are validated to contain `{{{ SCRIPT }}}` at build time
...
| WEB-RUNTIME-04 | Phase 3 — Runtime Audit | Complete |
| WEB-RUNTIME-05 | Phase 4 — WASM-Safe Time Shim | Complete |
| WEB-RUNTIME-06 | Phase 4 — WASM-Safe Time Shim | Complete |
| WEB-RUNTIME-07 | Phase 3 — Runtime Audit | Complete |
| WEB-EMIT-01 | Phase 5 — LIR Main-Loop Split Pass | Complete |
...
| WEB-SHELL-06 | Phase 9 — Shell Template + Audio Unlock | Complete (09-plan-02) |
