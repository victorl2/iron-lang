...
- [x] **FIX-02**: Every unchecked `malloc`/`calloc`/`realloc`/`strdup` call that could produce a user-visible crash is either fixed with a guarded error path (returning a typed error) or explicitly annotated `/* SAFETY: cannot fail because ... */` with the invariant documented at the call site
...
| FIX-02 | Phase 67 | In Progress (partial: 67-02 iron_runtime.h + emit_c.c + emit_web.c; 67-04 parser.c + lexer.c walkthrough complete (123 sites); 67-05 analyzer + comptime walkthrough complete (94 sites across 11 files); 67-06 continues hir+lir+runtime/stdlib walkthrough) |
