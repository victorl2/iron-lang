...
static bool block_is_terminated(IronLIR_Block *block) {
    ...
    return iron_lir_is_terminator(block->instrs[block->instr_count - 1]->kind);
}
...
void lower_stmt(IronLIR_LowerCtx *ctx, Iron_Node *node) {
    ...
    switch (node->kind) {
        ...
        case IRON_NODE_VAL_DECL: {
            ...
            IronLIR_ValueId init_val = IRON_LIR_VALUE_INVALID;
        ...
        case IRON_NODE_VAR_DECL: {
            ...
            IronLIR_Instr *slot  = iron_lir_alloca(fn, entry,
                                                  vd->declared_type, vd->name,
                                                  vd->span);
            shput(ctx->var_alloca_map, vd->name, slot->id);
            if (vd->init) {
                IronLIR_ValueId init_val = lower_expr(ctx, vd->init);
                iron_lir_store(fn, ctx->current_block, slot->id, init_val, vd->span);
            }
            break;
        }
        ...
        case IRON_NODE_ASSIGN: {
            ...
            IronLIR_ValueId rhs = lower_expr(ctx, a->value);
            if (a->target->kind == IRON_NODE_IDENT) {
                Iron_Ident *id = (Iron_Ident *)a->target;
                IronLIR_ValueId slot = shget(ctx->var_alloca_map, id->name);
                iron_lir_store(fn, ctx->current_block, slot, rhs, span);
            } else if (a->target->kind == IRON_NODE_FIELD_ACCESS) {
                Iron_FieldAccess *fa = (Iron_FieldAccess *)a->target;
                IronLIR_ValueId obj = lower_expr(ctx, fa->object);
                iron_lir_set_field(fn, ctx->current_block, obj, fa->field, rhs, span);
                ...
                if (fa->object->kind == IRON_NODE_IDENT) {
                    ...
                    if (slot_idx >= 0) {
                        IronLIR_ValueId slot = ctx->var_alloca_map[slot_idx].value;
                        iron_lir_store(fn, ctx->current_block, slot, obj, span);
                    }
                }
            } else if (a->target->kind == IRON_NODE_INDEX) {
                Iron_IndexExpr *idx = (Iron_IndexExpr *)a->target;
                IronLIR_ValueId arr   = lower_expr(ctx, idx->object);
                IronLIR_ValueId index = lower_expr(ctx, idx->index);
                iron_lir_set_index(fn, ctx->current_block, arr, index, rhs, span);
                ...
                if (idx->object->kind == IRON_NODE_IDENT) {
                    ...
                    if (slot_idx >= 0) {
                        IronLIR_ValueId slot = ctx->var_alloca_map[slot_idx].value;
                        iron_lir_store(fn, ctx->current_block, slot, arr, span);
                    }
                }
            } else {
        ...
        case IRON_NODE_IF: {
            ...
            IronLIR_Block *merge_block = new_block(ctx, "if_merge");

            /* Lower the primary condition */
            IronLIR_ValueId cond_val = lower_expr(ctx, is->condition);
            ...
            IronLIR_Block *then_block = new_block(ctx, "if_then");

            /* Determine the "else target" for the primary condition */
            IronLIR_Block *else_target;
            if (is->elif_count > 0) {
            ...
            /* Emit primary branch */
            iron_lir_branch(fn, ctx->current_block, cond_val,
            ...
            if (!block_is_terminated(ctx->current_block)) {
                iron_lir_jump(fn, ctx->current_block, merge_block->id, span);
            }

            /* Lower elif chains */
            IronLIR_Block *cur_elif_cond_block = else_target;
            for (int i = 0; i < is->elif_count; i++) {
                ...
                IronLIR_ValueId elif_cond = lower_expr(ctx, is->elif_conds[i]);

                IronLIR_Block *elif_then = new_block(ctx, "if_elif_then");

                IronLIR_Block *next_else;
                ...
                iron_lir_branch(fn, cur_elif_cond_block, elif_cond,
        ...
        case IRON_NODE_WHILE: {
            ...
            IronLIR_Block *header = new_block(ctx, "while_header");
            IronLIR_Block *body   = new_block(ctx, "while_body");
            IronLIR_Block *exit   = new_block(ctx, "while_exit");

            /* Jump from current block to header */
            iron_lir_jump(fn, ctx->current_block, header->id, span);
            ...
            IronLIR_ValueId cond_val = lower_expr(ctx, ws->condition);
            iron_lir_branch(fn, ctx->current_block, cond_val, body->id, exit->id, span);

            /* Save outer loop context */
            IronLIR_Block *old_exit     = ctx->loop_exit_block;
            IronLIR_Block *old_continue = ctx->loop_continue_block;
        ...
        case IRON_NODE_FOR: {
            ...
            if (fs->is_parallel) {
                 ...
                 * Push a LiftPending descriptor and emit IRON_LIR_PARALLEL_FOR. */
                ...
                IronLIR_ValueId range_val = lower_expr(ctx, fs->iterable);
                iron_lir_parallel_for(fn, ctx->current_block,
                                     fs->var_name, range_val, chunk_name,
                                     IRON_LIR_VALUE_INVALID,  /* pool_val: default */
                                     NULL, 0, span);
            } else {
                ...
                if (is_array_iter) {
                    ...
                    IronLIR_Instr *idx_slot = iron_lir_alloca(fn, entry,
                                                              int_type, idx_name, span);

                    /* Alloca for the item (var_name) */
                    Iron_Type *elem_type = iter_type->array.elem;
                    IronLIR_Instr *item_slot = iron_lir_alloca(fn, entry,
                                                               elem_type, fs->var_name, span);
                    shput(ctx->var_alloca_map, fs->var_name, item_slot->id);

                    IronLIR_Block *pre_header = new_block(ctx, "for_init");
                    IronLIR_Block *header     = new_block(ctx, "for_header");
                    IronLIR_Block *body_blk   = new_block(ctx, "for_body");
                    IronLIR_Block *increment  = new_block(ctx, "for_incr");
                    IronLIR_Block *exit_blk   = new_block(ctx, "for_exit");

                    iron_lir_jump(fn, ctx->current_block, pre_header->id, span);

                    /* Pre-header: store 0 into index */
                    switch_block(ctx, pre_header);
                    IronLIR_Instr *zero = iron_lir_const_int(fn, pre_header, 0, int_type, span);
                    iron_lir_store(fn, pre_header, idx_slot->id, zero->id, span);
                    /* Evaluate array once in pre_header */
                    IronLIR_ValueId arr_val = lower_expr(ctx, fs->iterable);
                    iron_lir_jump(fn, pre_header, header->id, span);

                    /* Header: load index, get arr.count, compare */
                    switch_block(ctx, header);
                    IronLIR_Instr *idx_cur = iron_lir_load(fn, header, idx_slot->id, int_type, span);
                    /* Access arr.count field for the bound */
                    IronLIR_Instr *bound_instr = iron_lir_get_field(fn, header,
                                                                    arr_val, "count",
                                                                    int_type, span);
                    IronLIR_Instr *cmp = iron_lir_binop(fn, header, IRON_LIR_LT,
                                                       idx_cur->id, bound_instr->id,
                                                       bool_type, span);
                    iron_lir_branch(fn, header, cmp->id, body_blk->id, exit_blk->id, span);
                    ...
                    IronLIR_Instr *idx_body = iron_lir_load(fn, body_blk,
                                                           idx_slot->id, int_type, span);
                    IronLIR_Instr *item = iron_lir_get_index(fn, body_blk,
                                                            arr_val, idx_body->id,
                                                            elem_type, span);
                    iron_lir_store(fn, body_blk, item_slot->id, item->id, span);
                    lower_block(ctx, (Iron_Block *)fs->body);
                    if (!block_is_terminated(ctx->current_block)) {
                        iron_lir_jump(fn, ctx->current_block, increment->id, span);
                    }

                    /* Increment: load idx, add 1, store */
                    switch_block(ctx, increment);
                    IronLIR_Instr *idx_inc = iron_lir_load(fn, increment, idx_slot->id, int_type, span);
                    IronLIR_Instr *one_inc = iron_lir_const_int(fn, increment, 1, int_type, span);
                    IronLIR_Instr *next_idx = iron_lir_binop(fn, increment, IRON_LIR_ADD,
                                                            idx_inc->id, one_inc->id,
                                                            int_type, span);
                    iron_lir_store(fn, increment, idx_slot->id, next_idx->id, span);
                    iron_lir_jump(fn, increment, header->id, span);
                    ...
                    IronLIR_Block *body_blk   = new_block(ctx, "for_body");
                    IronLIR_Block *increment  = new_block(ctx, "for_incr");
                    IronLIR_Block *exit       = new_block(ctx, "for_exit");
                    ...
                    switch_block(ctx, pre_header);
                    IronLIR_Instr *zero = iron_lir_const_int(fn, pre_header, 0, int_type, span);
                    iron_lir_store(fn, pre_header, slot->id, zero->id, span);
                    IronLIR_ValueId bound  = lower_expr(ctx, fs->iterable);
                    iron_lir_jump(fn, pre_header, header->id, span);

                    /* Header: load counter, compare against pre-computed bound */
                    switch_block(ctx, header);
                    IronLIR_Instr *counter = iron_lir_load(fn, header, slot->id, int_type, span);
                    IronLIR_Instr *cmp     = iron_lir_binop(fn, header, IRON_LIR_LT,
                                                           counter->id, bound,
                                                           bool_type, span);
                    iron_lir_branch(fn, header, cmp->id, body_blk->id, exit->id, span);
                    ...
                    switch_block(ctx, increment);
                    IronLIR_Instr *cur_val = iron_lir_load(fn, increment, slot->id, int_type, span);
                    IronLIR_Instr *one     = iron_lir_const_int(fn, increment, 1, int_type, span);
                    IronLIR_Instr *next_val = iron_lir_binop(fn, increment, IRON_LIR_ADD,
                                                            cur_val->id, one->id,
                                                            int_type, span);
                    iron_lir_store(fn, increment, slot->id, next_val->id, span);
                    iron_lir_jump(fn, increment, header->id, span);
        ...
        case IRON_NODE_MATCH: {
            ...
            /* Lower subject */
            IronLIR_ValueId subject_val = lower_expr(ctx, ms->subject);

            /* Create join block */
            IronLIR_Block *join_block = new_block(ctx, "match_join");

            /* Create default block (else body or direct jump to join) */
            IronLIR_Block *default_block;
            if (ms->else_body) {
            ...
            IronLIR_BlockId *case_blocks = NULL;  /* stb_ds dynamic array */

            for (int i = 0; i < ms->case_count; i++) {
                Iron_MatchCase *mc = (Iron_MatchCase *)ms->cases[i];
                IronLIR_Block *arm_block = new_block(ctx, "match_arm");
            ...
            iron_lir_switch(fn, ctx->current_block, subject_val,
                           default_block->id,
            ...
            for (int i = 0; i < ms->case_count; i++) {
                ...
                /* case_blocks[i] is the BlockId — look up the IronLIR_Block* */
                IronLIR_Block *arm_block = NULL;
                ...
                if (ctx->current_block && !block_is_terminated(ctx->current_block)) {
                    iron_lir_jump(fn, ctx->current_block, join_block->id, span);
                }
            }
        ...
        case IRON_NODE_RETURN: {
            ...
            if (rs->value) {
                IronLIR_ValueId ret_val = lower_expr(ctx, rs->value);
                iron_lir_return(fn, ctx->current_block, ret_val, false,
                               fn->return_type, span);
            } else {
                iron_lir_return(fn, ctx->current_block, IRON_LIR_VALUE_INVALID,
                               true, NULL, span);
            }
        ...
        case IRON_NODE_FREE: {
            ...
            IronLIR_ValueId val = lower_expr(ctx, fs->expr);
            iron_lir_free(fn, ctx->current_block, val, span);
            break;
        }
        ...
        case IRON_NODE_SPAWN: {
            ...
            IronLIR_ValueId pool_val = IRON_LIR_VALUE_INVALID;
            if (ss->pool_expr) {
                pool_val = lower_expr(ctx, ss->pool_expr);
            }

            /* Emit IRON_LIR_SPAWN instruction */
            Iron_Type *void_type = iron_type_make_primitive(IRON_TYPE_VOID);
            iron_lir_spawn(fn, ctx->current_block,
