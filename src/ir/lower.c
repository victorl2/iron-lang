...
/* ── Stub lower_stmt ─────────────────────────────────────────────────────── */
/* Full implementation in Plan 08-02 (lower_stmts.c).
 * This stub handles only the statement kinds needed for expression testing. */
    ...
    switch (node->kind) {
        ...
        case IRON_NODE_VAR_DECL: {
            ...
            IronIR_Instr *slot = iron_ir_alloca(ctx->current_func, entry,
                                                 vd->declared_type, vd->name, vd->span);
        ...
        case IRON_NODE_RETURN: {
            ...
            if (rs->value) {
                IronIR_ValueId val = lower_expr(ctx, rs->value);
                iron_ir_return(ctx->current_func, ctx->current_block,
                               val, false, ctx->current_func->return_type, rs->span);
            } else {
                iron_ir_return(ctx->current_func, ctx->current_block,
                               IRON_IR_VALUE_INVALID, true, NULL, rs->span);
            }
            break;
        }
        case IRON_NODE_DEFER: {
            Iron_DeferStmt *ds = (Iron_DeferStmt *)node;
            /* Push the deferred expression onto the current defer stack level */
            if (ctx->defer_depth > 0) {
        ...
        case IRON_NODE_FREE: {
            ...
            iron_ir_free(ctx->current_func, ctx->current_block, val, fs->span);
            break;
        }
        default:
