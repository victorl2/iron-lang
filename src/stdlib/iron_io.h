...
#ifdef IRON_IO_GENERATED_OBJECTS
typedef struct Iron_FileReadResult Iron_FileReadResult;
typedef struct Iron_FileWriteResult Iron_FileWriteResult;
typedef struct Iron_FileInfo Iron_FileInfo;
#else
typedef struct Iron_FileReadResult {
    Iron_String data;
    int64_t error;
    Iron_String error_message;
} Iron_FileReadResult;

typedef struct Iron_FileWriteResult {
    int64_t bytes;
    int64_t error;
    Iron_String error_message;
} Iron_FileWriteResult;

typedef struct Iron_FileInfo {
    bool exists;
    bool is_file;
    bool is_dir;
    int64_t size;
...
static inline Iron_String Iron_io_read_bytes_legacy(Iron_String path) {
...
/* Binary-safe, bounded, error-reporting file surface. */
