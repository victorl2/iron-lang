 ...
 *
 *   Iron_Arena_RT *iron_arena_new(uint64_t capacity, bool threadsafe,
 *                                 const char *name);
 *   Iron_FatPtr    iron_arena_alloc(Iron_Arena_RT *a, uint64_t size);
 *   void           iron_arena_reset(Iron_Arena_RT *a);            // ARENA-06
 *   Iron_ArenaSave iron_arena_save(Iron_Arena_RT *a);             // ARENA-01
 *   void           iron_arena_restore(Iron_Arena_RT *a, Iron_ArenaSave s); // ARENA-07
 *   uint64_t       iron_arena_used(Iron_Arena_RT *a);             // ARENA-01
 *   uint64_t       iron_arena_capacity(Iron_Arena_RT *a);         // ARENA-01
 *   void           iron_arena_destroy(Iron_Arena_RT *a);
 *
...
void test_arena_new_used_capacity(void) {
    Iron_Arena_RT *a = iron_arena_new(4096, false, "t");
    TEST_ASSERT_NOT_NULL(a);
    TEST_ASSERT_EQUAL_UINT64(4096, iron_arena_capacity(a));
    TEST_ASSERT_EQUAL_UINT64(0, iron_arena_used(a));
...
void test_arena_alloc_bumps(void) {
    ...
    uint64_t u0 = iron_arena_used(a);
    Iron_FatPtr p1 = iron_arena_alloc(a, 64);
    uint64_t u1 = iron_arena_used(a);
    Iron_FatPtr p2 = iron_arena_alloc(a, 64);
    uint64_t u2 = iron_arena_used(a);
...
void test_arena_reset_zeroes_used(void) {
    ...
    (void)iron_arena_alloc(a, 128);
    TEST_ASSERT_TRUE(iron_arena_used(a) > 0);
...
void test_arena_save_restore_offset(void) {
    ...
    (void)iron_arena_alloc(a, 64);
    uint64_t o1 = iron_arena_used(a);
    Iron_ArenaSave save = iron_arena_save(a);

    (void)iron_arena_alloc(a, 64);
    (void)iron_arena_alloc(a, 64);
    TEST_ASSERT_TRUE(iron_arena_used(a) > o1);

    iron_arena_restore(a, save);
    TEST_ASSERT_EQUAL_UINT64(o1, iron_arena_used(a));
...
void test_arena_threadsafe_variant(void) {
    ...
    Iron_Arena_RT *a = iron_arena_new(1 << 20, true, "ts");
    TEST_ASSERT_NOT_NULL(a);

    const int N = 1000;
    uint64_t prev_used = iron_arena_used(a);
    char *prev_addr = NULL;
    for (int i = 0; i < N; i++) {
        Iron_FatPtr fp = iron_arena_alloc(a, 8);
        TEST_ASSERT_NOT_NULL(fp.addr);
        uint64_t cur_used = iron_arena_used(a);
    ...
    TEST_ASSERT_TRUE(iron_arena_used(a) >= (uint64_t)N * 8);
