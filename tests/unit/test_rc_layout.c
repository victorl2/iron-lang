/* test_rc_layout.c — Phase 26 POL-06 ABI layout invariants.
 *
 * Wave 0 RED for Plan 26-01 Task 1; turns GREEN after Plan 26-01 Task 2 lands
 * the iron_rc_alloc / iron_rc_retain / iron_rc_release implementation.
 *
 ...
 *   - sizeof(Iron_RcHeader) == 16  (ABI lock, also _Static_assert in
...
void test_iron_rc_header_size_16_bytes(void) {
    /* CONTEXT.md GA1: 16B on 64-bit POSIX/Win32. Locked at TU scope in
     * iron_runtime.h via _Static_assert; this runtime probe catches any
     * accidental layout change that slips past the compile-time guard
     * (e.g., #pragma pack manipulation in a downstream TU). */
    TEST_ASSERT_EQUAL_UINT64(16, (uint64_t)sizeof(Iron_RcHeader));
}
...
void test_iron_rc_header_refcount_at_offset_0(void) {
     ...
     * the refcount to sit at the block prefix. Phase 27 weak rc may append
     * a weak_count field; the offset 0 lock on refcount is permanent. */
...
int main(void) {
    UNITY_BEGIN();
    RUN_TEST(test_iron_rc_header_size_16_bytes);
